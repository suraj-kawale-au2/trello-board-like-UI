import React from 'react';
import { MDBNavbar, MDBNavbarBrand, MDBNavbarNav, MDBNavbarToggler, MDBCollapse, MDBNavItem, MDBBtn , MDBIcon } from 'mdbreact';


class Header extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
          collapse: false,
          isWideEnough: false,
        };
        this.onClick = this.onClick.bind(this);
    }

    onClick() {
        this.setState({
          collapse: !this.state.collapse,
        });
      }

    render(){
        return(
            
            <MDBNavbar color="elegant" fixed="top" dark expand="md" transparent className="shadow-sm mb-5">
              
              {!this.state.isWideEnough && <MDBNavbarToggler onClick={this.onClick} />}
              <MDBCollapse isOpen={this.state.collapse} navbar>
                <MDBNavbarNav left>
                  <MDBNavItem className="ml-1">
                  <MDBBtn outline color="light-blue" className="text-white shadow-sm"><MDBIcon icon="home" /></MDBBtn>
                  </MDBNavItem>
                  <MDBNavItem className="ml-1">
                  <MDBBtn outline color="light-blue" className="text-white shadow-sm"><MDBIcon icon="trello" /> Boards</MDBBtn>
                  </MDBNavItem>
                  <MDBNavItem className="ml-1">
                  <div className="input-group md-form form-sm shadow-sm">
                        
                        <input className="form-control bg-transparent text-white rounded-left" type="text" />
                        <div className="input-group-prepend">
                            <MDBBtn  color="" className="text-white rounded-right">
                                <MDBIcon icon="search" />
                            </MDBBtn>
                        </div>
                   </div>
                  </MDBNavItem>
                </MDBNavbarNav>
                <MDBNavbarBrand>
                  <img src="https://a.trellocdn.com/prgb/dist/images/header-logo-2x.01ef898811a879595cea.png" alt="" height="30px"/>
              </MDBNavbarBrand>
              <MDBNavbarNav right>
                    <MDBNavItem className="ml-1">
                        <MDBBtn outline color="light-blue" className="text-white shadow-sm"><MDBIcon icon="plus" /></MDBBtn>
                    </MDBNavItem>
                    <MDBNavItem className="ml-1">
                        <MDBBtn outline color="light-blue" className="text-white shadow-sm"><MDBIcon icon="info" /></MDBBtn>
                    </MDBNavItem>
                    <MDBNavItem className="ml-1">
                        <MDBBtn outline color="light-blue" className="text-white shadow-sm"><MDBIcon icon="bell" /></MDBBtn>
                    </MDBNavItem>
                    <MDBNavItem className="ml-1">
                        <img src="data:image/png;base64," alt="" className="rounded-circle" width="35px" height="35px"/>
                    </MDBNavItem>

                </MDBNavbarNav>
              </MDBCollapse>
            </MDBNavbar>
          
        )
    }

}

export default Header;